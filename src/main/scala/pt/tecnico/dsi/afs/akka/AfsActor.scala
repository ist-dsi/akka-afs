...
import pt.tecnico.dsi.afs.akka.AfsActor.{RemoveResult, Retry, SaveSnapshot, SideEffectResult}
...
/**
  *
  */
class AfsActor(val settings: Settings = new Settings()) extends Actor with PersistentActor with ActorLogging {

  def persistenceId: String = "afsActor"

  import settings._


  private val blockingActor = context.actorOf(Props(classOf[BlockingActor], afsSettings))
